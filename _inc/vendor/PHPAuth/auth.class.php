...
class Auth
{
	...
	*/
	...
	public function login($email, $password, $remember = 0, $username_log = true )
	{
		$return['error'] = true;
		...
		if( $username_log === TRUE ){
			$uid = $this->getUID( strtolower( $email ) );
			if( !$uid ) {
				$uid = $this->getUIDByUsername( $email );
				if( $uid ) $validateEmail = ['error' => 0];
			};
		}
    ...
    */
    ...
    {
        return substr(strtr(base64_encode(random_bytes(22)), '+', '.'), 0, 22);
    }
	...
	*/
	...
	private function addUser($email, $username, $password, $user_roles = 'user')
	{
		$return['error'] = true;
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, username = ?, password = ?, isactive = ?, user_roles = ? WHERE id = ?");

		if(!$query->execute(array($email, $username, $password, 0, $user_roles, $uid))) {
			$query = $this->dbh->prepare("DELETE FROM {$this->config->table_users} WHERE id = ?");
			$query->execute(array($uid));
	...
	*/
	...
	{
		$query = $this->dbh->prepare("SELECT email, username, password, isactive, user_roles FROM {$this->config->table_users} WHERE id = ?");
		$query->execute(array($uid));
